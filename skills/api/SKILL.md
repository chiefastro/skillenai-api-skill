...
| `scripts/openreview_collect.py` | Peer reviews and meta-reviews from OpenReview with no account. OpenReview's `/notes` endpoint is behind a bot challenge (403 ChallengeRequiredError); `/notes/search` is not, and returns full review text plus structured scores. Handles the consequences: search REQUIRES a query term (so corpora are term-assembled and deduped, valid for per-review text properties but NOT for venue totals), `offset` caps at 10,000, and `forum=` is silently ignored so per-paper retrieval is impossible. `--kind meta` swaps paper vocabulary for chair-voice terms, which lifts meta-review yield from ~0.8% to ~26%. `--estimate-volume` recovers venue size via the German tank estimator on submission numbers embedded in reviewer signatures. Fails loudly on repeated request failure rather than recording a dead request as an empty page. |
